# 🚀 微信小程序新年抽签应用 - 部署指南

## 📋 部署概览

本指南将帮助您将微信小程序新年抽签应用部署到生产环境。我们提供多种部署方案，包括云服务器部署、NAS环境部署和本地开发部署。

## 🎯 部署架构对比

### 部署方式选择

| 部署方式 | 适用场景 | 端口配置 | 配置文件 | 主要特点 |
|---------|---------|---------|----------|----------|
| **开发环境** | 本地开发调试 | 3000 | `docker-compose.dev.yml` | 热重载、调试模式、开发工具 |
| **生产环境** | 云服务器部署 | 3000 | `docker-compose.prod.yml` | Cloudflare隧道、性能优化 |
| **🆕 NAS环境** | **NAS系统部署** | **18080** | **`docker-compose.nas.yml`** | **高端口、资源优化、NAS适配** |

### 推荐架构
```
# 云服务器部署架构
用户 → 微信小程序 → Cloudflare → 云服务器 → Docker容器 → SQLite数据库

# NAS环境部署架构（新增）
用户 → 微信小程序 → NAS网络 → Docker容器 → SQLite数据库
```

### 组件说明
- **微信小程序**: 前端用户界面
- **Cloudflare**: CDN加速和安全防护（云服务器部署）
- **云服务器/NAS**: 运行后端服务的硬件平台
- **Docker容器**: 应用容器化运行
- **SQLite数据库**: 数据持久化存储

## 🛠️ 环境准备

### 服务器要求
- **CPU**: 2核心以上
- **内存**: 4GB以上
- **存储**: 40GB以上SSD
- **网络**: 5Mbps以上带宽
- **系统**: Ubuntu 20.04 LTS（推荐）

### 软件依赖
- Docker 20.10+
- Docker Compose 2.0+
- Git 2.25+
- Nginx 1.18+（可选）

## 🔧 快速部署

### 🆕 NAS环境部署（推荐新用户）

#### 新增NAS专用配置

项目现在提供了专门为NAS环境优化的部署配置：

**配置文件：**
- **`docker-compose.nas.yml`** - NAS专用Docker Compose配置
- **`.env.nas.example`** - NAS环境变量模板

**NAS配置优化特性：**
1. **端口优化**：使用18080/18082高端口避免与NAS现有服务冲突
2. **资源管理**：针对NAS硬件特点的CPU和内存限制配置
3. **安全增强**：只读根文件系统、安全选项配置
4. **性能优化**：SQLite WAL模式、数据库缓存优化
5. **监控完善**：独立健康检查脚本和日志管理
6. **数据持久化**：优化的数据卷挂载策略
7. **环境适配**：支持群晖、威联通等主流NAS系统

#### NAS快速部署步骤

```bash
# 1. 准备环境配置
cp .env.nas.example .env.nas
# 编辑 .env.nas 文件，设置正确的NAS路径和端口

# 2. 创建NAS数据目录
mkdir -p /volume1/docker/wechat-fortune-draw/{data,logs}
chmod 755 /volume1/docker/wechat-fortune-draw

# 3. 启动NAS服务
docker-compose -f docker-compose.nas.yml --env-file .env.nas up -d

# 4. 验证NAS服务
curl http://localhost:18080/api/health
```

#### NAS访问地址
- **后端API**: `http://NAS_IP:18080`
- **数据库管理**: `http://NAS_IP:18082`（可选的Adminer工具）
- **健康检查**: `http://NAS_IP:18080/api/health`

### 云服务器部署（传统方式）

### 1. 服务器初始化
```bash
# 更新系统
sudo apt update && sudo apt upgrade -y

# 安装基础工具
sudo apt install -y curl wget git vim htop

# 安装Docker
curl -fsSL https://get.docker.com -o get-docker.sh
sudo sh get-docker.sh
sudo usermod -aG docker $USER

# 安装Docker Compose
sudo curl -L "https://github.com/docker/compose/releases/download/v2.20.0/docker-compose-$(uname -s)-$(uname -m)" -o /usr/local/bin/docker-compose
sudo chmod +x /usr/local/bin/docker-compose

# 重新登录以应用Docker组权限
exit
```

### 2. 部署应用
```bash
# 克隆项目
git clone https://github.com/your-username/wechat-fortune-draw.git
cd wechat-fortune-draw

# 配置环境变量
cp .env.docker .env
vim .env  # 编辑配置文件

# 设置数据目录权限
mkdir -p data logs
sudo chown -R 1001:1001 data logs

# 启动服务（开发环境）
docker-compose -f docker-compose.dev.yml up -d

# 或启动生产环境（需要配置TUNNEL_TOKEN）
docker-compose -f docker-compose.prod.yml up -d

# 查看服务状态
docker-compose ps
```

### 3. 验证部署
```bash
# 检查服务健康状态
curl http://localhost:3000/api/health

# 查看日志
docker-compose logs -f
```

## 📝 配置详解

### 环境变量配置
```env
# .env - 主配置文件
# 后端服务端口
BACKEND_PORT=3000

# 数据持久化目录
DATA_PATH=./data

# 运行环境
NODE_ENV=production

# Cloudflare Tunnel Token（生产环境必需）
TUNNEL_TOKEN=your_tunnel_token_here

# 允许的跨域来源
CORS_ORIGINS=https://servicewechat.com,https://your-domain.com

# 微信小程序配置（可选）
WECHAT_APP_ID=your_wechat_app_id
WECHAT_APP_SECRET=your_wechat_app_secret

# 安全配置
JWT_SECRET=your_super_secret_jwt_key_here

# 日志配置
LOG_LEVEL=info
```
### Docker Compose配置

#### 主配置文件 (docker-compose.yml)
```yaml
# docker-compose.yml - 基础配置
version: '3.8'

services:
  fortune-backend:
    build:
      context: ./backend
      dockerfile: Dockerfile
      target: runtime
    image: wechat-fortune-draw:latest
    container_name: fortune-backend
    restart: unless-stopped
    
    ports:
      - "${BACKEND_PORT:-3000}:3000"
    
    environment:
      - NODE_ENV=production
      - PORT=3000
      - DB_PATH=/app/data/fortune.db
    
    volumes:
      - fortune_data:/app/data
      - ./backend/logs:/app/logs:rw
    
    healthcheck:
      test: ["CMD", "node", "-e", "const http = require('http'); const options = { host: 'localhost', port: 3000, path: '/api/health', timeout: 2000 }; const req = http.request(options, (res) => { if (res.statusCode === 200) { process.exit(0); } else { process.exit(1); } }); req.on('error', () => process.exit(1)); req.on('timeout', () => process.exit(1)); req.end();"]
      interval: 30s
      timeout: 10s
      retries: 3
      start_period: 10s
    
    deploy:
      resources:
        limits:
          cpus: '0.5'
          memory: 512M
        reservations:
          cpus: '0.1'
          memory: 128M
    
    networks:
      - fortune-network
    
    security_opt:
      - no-new-privileges:true
    read_only: true
    tmpfs:
      - /tmp:noexec,nosuid,size=100m

volumes:
  fortune_data:
    driver: local
    driver_opts:
      type: none
      o: bind
      device: ${DATA_PATH:-./data}

networks:
  fortune-network:
    driver: bridge
    ipam:
      config:
        - subnet: 172.20.0.0/16
```

#### 生产环境配置 (docker-compose.prod.yml)
```yaml
# docker-compose.prod.yml - 生产环境配置
version: '3.8'

services:
  fortune-backend:
    build:
      context: ./backend
      dockerfile: Dockerfile
      target: runtime
    image: wechat-fortune-draw:latest
    container_name: fortune-backend-prod
    restart: unless-stopped
    
    # 仅内部网络访问，通过Cloudflare Tunnel对外提供服务
    expose:
      - "3000"
    
    environment:
      - NODE_ENV=production
      - PORT=3000
      - DB_PATH=/app/data/fortune.db
      - CORS_ORIGINS=${CORS_ORIGINS:-https://servicewechat.com}
    
    volumes:
      - fortune_data:/app/data
      - ./logs:/app/logs:rw
    
    deploy:
      resources:
        limits:
          cpus: '1.0'
          memory: 1G
        reservations:
          cpus: '0.2'
          memory: 256M
    
    networks:
      - fortune-internal
    
    security_opt:
      - no-new-privileges:true
    read_only: true
    tmpfs:
      - /tmp:noexec,nosuid,size=100m

  # Cloudflare Tunnel 服务
  cloudflare-tunnel:
    image: cloudflare/cloudflared:latest
    container_name: fortune-cloudflare-tunnel
    restart: unless-stopped
    command: tunnel --no-autoupdate run --token ${TUNNEL_TOKEN}
    environment:
      - TUNNEL_TOKEN=${TUNNEL_TOKEN}
    depends_on:
      fortune-backend:
        condition: service_healthy
    networks:
      - fortune-internal
    security_opt:
      - no-new-privileges:true
    read_only: true

volumes:
  fortune_data:
    driver: local
    driver_opts:
      type: none
      o: bind
      device: ${DATA_PATH:-./data}

networks:
  fortune-internal:
    driver: bridge
    internal: false
    ipam:
      config:
        - subnet: 172.21.0.0/16
```

#### 开发环境配置 (docker-compose.dev.yml)
```yaml
# docker-compose.dev.yml - 开发环境配置
version: '3.8'

services:
  fortune-backend-dev:
    build:
      context: ./backend
      dockerfile: Dockerfile
      target: builder  # 使用构建阶段，包含开发工具
    image: wechat-fortune-draw:dev
    container_name: fortune-backend-dev
    restart: unless-stopped
    
    ports:
      - "3000:3000"
    
    environment:
      - NODE_ENV=development
      - PORT=3000
      - DB_PATH=/app/data/fortune.db
    
    volumes:
      - ./backend:/app:rw
      - /app/node_modules  # 匿名卷，避免覆盖容器内的node_modules
      - ./data:/app/data:rw
      - ./backend/logs:/app/logs:rw
    
    command: ["npm", "run", "dev"]
    
    networks:
      - fortune-dev-network

networks:
  fortune-dev-network:
    driver: bridge
```

## 🌐 域名和HTTPS配置

### 1. 域名解析
在您的域名服务商处添加A记录：
```
类型: A
主机记录: api (或 @)
记录值: 您的服务器IP地址
TTL: 600
```

### 2. Nginx反向代理
```nginx
# /etc/nginx/sites-available/fortune-app
server {
    listen 80;
    server_name your-domain.com;
    
    # 重定向到HTTPS
    return 301 https://$server_name$request_uri;
}

server {
    listen 443 ssl http2;
    server_name your-domain.com;
    
    # SSL证书配置
    ssl_certificate /path/to/your/certificate.crt;
    ssl_certificate_key /path/to/your/private.key;
    
    # SSL安全配置
    ssl_protocols TLSv1.2 TLSv1.3;
    ssl_ciphers ECDHE-RSA-AES256-GCM-SHA512:DHE-RSA-AES256-GCM-SHA512;
    ssl_prefer_server_ciphers off;
    
    # 反向代理配置
    location / {
        proxy_pass http://localhost:3000;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        
        # 超时配置
        proxy_connect_timeout 60s;
        proxy_send_timeout 60s;
        proxy_read_timeout 60s;
    }
    
    # 静态文件缓存
    location ~* \.(js|css|png|jpg|jpeg|gif|ico|svg)$ {
        expires 1y;
        add_header Cache-Control "public, immutable";
    }
}
```

### 3. 启用Nginx配置
```bash
# 创建软链接
sudo ln -s /etc/nginx/sites-available/fortune-app /etc/nginx/sites-enabled/

# 测试配置
sudo nginx -t

# 重启Nginx
sudo systemctl restart nginx
```

### 4. SSL证书申请（Let's Encrypt）
```bash
# 安装Certbot
sudo apt install certbot python3-certbot-nginx

# 申请证书
sudo certbot --nginx -d your-domain.com

# 设置自动续期
sudo crontab -e
# 添加以下行：
# 0 12 * * * /usr/bin/certbot renew --quiet
```

## 🔐 安全配置

### 1. 防火墙设置
```bash
# 启用UFW防火墙
sudo ufw enable

# 允许SSH
sudo ufw allow ssh

# 允许HTTP和HTTPS
sudo ufw allow 80
sudo ufw allow 443

# 查看状态
sudo ufw status
```

### 2. 系统安全加固
```bash
# 禁用root登录
sudo vim /etc/ssh/sshd_config
# 设置: PermitRootLogin no

# 创建普通用户
sudo adduser deploy
sudo usermod -aG sudo deploy
sudo usermod -aG docker deploy

# 重启SSH服务
sudo systemctl restart ssh
```

### 3. 应用安全配置
```bash
# 设置文件权限
chmod 600 backend/.env
chmod 755 data/
chmod 644 data/fortune.db

# 创建专用用户运行应用
sudo useradd -r -s /bin/false fortune-app
sudo chown -R fortune-app:fortune-app data/ logs/
```

## 📊 监控和日志

### 1. 系统监控
```bash
# 安装监控工具
sudo apt install htop iotop nethogs

# 查看系统资源
htop
df -h
free -h
```

### 2. 应用监控
```bash
# 查看Docker容器状态
docker stats

# 查看应用日志
docker-compose logs -f --tail=100

# 查看特定服务日志
docker-compose logs -f app
```

### 3. 日志管理
```bash
# 配置日志轮转
sudo vim /etc/logrotate.d/fortune-app

# 内容：
/path/to/logs/*.log {
    daily
    rotate 30
    compress
    delaycompress
    missingok
    notifempty
    create 644 fortune-app fortune-app
    postrotate
        docker-compose restart app
    endscript
}
```

## 💾 数据备份

### 1. 自动备份脚本
```bash
#!/bin/bash
# backup.sh

# 配置
BACKUP_DIR="/home/deploy/backups"
DB_PATH="/home/deploy/wechat-fortune-draw/data/fortune.db"
DATE=$(date +%Y%m%d_%H%M%S)
BACKUP_FILE="fortune_backup_$DATE.db"

# 创建备份目录
mkdir -p $BACKUP_DIR

# 备份数据库
cp $DB_PATH $BACKUP_DIR/$BACKUP_FILE

# 压缩备份
gzip $BACKUP_DIR/$BACKUP_FILE

# 删除30天前的备份
find $BACKUP_DIR -name "fortune_backup_*.db.gz" -mtime +30 -delete

# 上传到云存储（可选）
# aws s3 cp $BACKUP_DIR/$BACKUP_FILE.gz s3://your-bucket/backups/

echo "备份完成: $BACKUP_FILE.gz"
```

### 2. 设置定时备份
```bash
# 添加执行权限
chmod +x backup.sh

# 设置定时任务
crontab -e

# 添加以下行（每天凌晨2点备份）
0 2 * * * /home/deploy/backup.sh >> /home/deploy/backup.log 2>&1
```

### 3. 数据恢复
```bash
# 停止应用
docker-compose down

# 恢复数据库
gunzip -c /path/to/backup/fortune_backup_20260106_020000.db.gz > data/fortune.db

# 重启应用
docker-compose up -d
```

## 🚀 性能优化

### 1. 数据库优化
```sql
-- 数据库维护脚本
-- optimize_db.sql

-- 重建索引
REINDEX;

-- 清理数据库
VACUUM;

-- 分析查询计划
ANALYZE;
```

### 2. 应用性能调优
```bash
# 调整Docker容器资源限制
# docker-compose.prod.yml
services:
  app:
    deploy:
      resources:
        limits:
          cpus: '2.0'
          memory: 2G
        reservations:
          cpus: '1.0'
          memory: 1G
```

### 3. 缓存配置
```nginx
# Nginx缓存配置
location /api/ {
    # API接口不缓存
    add_header Cache-Control "no-cache, no-store, must-revalidate";
    proxy_pass http://localhost:3000;
}

location /static/ {
    # 静态资源长期缓存
    expires 1y;
    add_header Cache-Control "public, immutable";
}
```

## 🔄 更新部署

### 1. 滚动更新
```bash
#!/bin/bash
# update.sh

echo "开始更新部署..."

# 拉取最新代码
git pull origin main

# 备份当前数据
./backup.sh

# 构建新镜像
docker-compose -f docker-compose.prod.yml build

# 重启服务
docker-compose -f docker-compose.prod.yml up -d

# 等待服务启动
sleep 30

# 健康检查
if curl -f http://localhost:3000/api/health; then
    echo "✅ 更新成功"
else
    echo "❌ 更新失败，开始回滚"
    # 这里可以添加回滚逻辑
fi
```

### 2. 蓝绿部署
```bash
# 蓝绿部署脚本
# blue-green-deploy.sh

# 当前运行的是蓝色环境，部署到绿色环境
docker-compose -f docker-compose.green.yml up -d

# 等待绿色环境启动
sleep 30

# 健康检查
if curl -f http://localhost:3001/api/health; then
    # 切换Nginx配置到绿色环境
    sudo cp nginx.green.conf /etc/nginx/sites-available/fortune-app
    sudo nginx -s reload
    
    # 停止蓝色环境
    docker-compose -f docker-compose.blue.yml down
    
    echo "✅ 蓝绿部署成功"
else
    echo "❌ 绿色环境启动失败"
    docker-compose -f docker-compose.green.yml down
fi
```

## 🐛 故障排除

### 常见问题

#### 1. 容器启动失败
```bash
# 查看详细错误信息
docker-compose logs app

# 检查配置文件
docker-compose config

# 检查端口占用
netstat -tlnp | grep :3000
```

#### 2. 数据库连接失败
```bash
# 检查数据库文件权限
ls -la data/fortune.db

# 检查数据库文件完整性
sqlite3 data/fortune.db "PRAGMA integrity_check;"

# 重新初始化数据库
rm data/fortune.db
docker-compose restart app
```

#### 3. 内存不足
```bash
# 查看内存使用
free -h
docker stats

# 添加交换空间
sudo fallocate -l 2G /swapfile
sudo chmod 600 /swapfile
sudo mkswap /swapfile
sudo swapon /swapfile

# 永久启用交换空间
echo '/swapfile none swap sw 0 0' | sudo tee -a /etc/fstab
```

#### 4. 磁盘空间不足
```bash
# 查看磁盘使用
df -h

# 清理Docker镜像
docker system prune -a

# 清理日志文件
sudo journalctl --vacuum-time=7d
```

## 📈 扩容方案

### 1. 垂直扩容
```bash
# 升级服务器配置
# 增加CPU和内存资源

# 调整Docker资源限制
# 修改docker-compose.prod.yml中的资源配置
```

### 2. 水平扩容
```yaml
# docker-compose.scale.yml
version: '3.8'

services:
  app:
    build: ./backend
    deploy:
      replicas: 3
    ports:
      - "3000-3002:3000"
    
  nginx:
    image: nginx:alpine
    ports:
      - "80:80"
      - "443:443"
    volumes:
      - ./nginx.conf:/etc/nginx/nginx.conf
    depends_on:
      - app
```

### 3. 负载均衡配置
```nginx
# nginx.conf
upstream fortune_backend {
    server localhost:3000;
    server localhost:3001;
    server localhost:3002;
}

server {
    listen 80;
    
    location / {
        proxy_pass http://fortune_backend;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
    }
}
```

## 📋 部署检查清单

### 部署前检查
- [ ] 服务器资源充足
- [ ] 域名解析正确
- [ ] SSL证书有效
- [ ] 环境变量配置完整
- [ ] 数据库备份完成

### 部署后验证
- [ ] 服务健康检查通过
- [ ] API接口响应正常
- [ ] 数据库连接正常
- [ ] 日志记录正常
- [ ] 监控告警配置

### 安全检查
- [ ] 防火墙规则正确
- [ ] SSL配置安全
- [ ] 文件权限合理
- [ ] 敏感信息保护
- [ ] 访问日志记录

---

**🎊 恭喜！您已成功部署微信小程序新年抽签应用！🎊**

如有问题，请参考故障排除章节或联系技术支持。