...
import { ValidationError, CooldownError, NotFoundError, DatabaseError } from '../utils/errors';
...
export class FortuneService {
    ...
    try {
      ...
      if (cooldownStatus.isInCooldown) {
        throw new CooldownError(
          `请等待 ${cooldownStatus.remainingSeconds} 秒后再抽签`,
  ...
  async performDraw(openid: string, options?: DrawOptions): Promise<DrawResult> {
    try {
      ...
      if (availableFortunes.length === 0) {
        throw new NotFoundError('恭喜您！已经抽完了所有运势，新年好运满满！');
      }
      ...
      if (error instanceof Error && error.message.includes('SQLITE_CONSTRAINT')) {
        throw new DatabaseError('抽签记录冲突，请重试');
      }
      
      throw new DatabaseError('抽签执行失败，请稍后重试');
    }
  }
  ...
  async drawFortune(openid: string): Promise<FortuneResponse> {
    try {
      // 验证抽签请求（包含冷却检查）
      await this.validateDrawRequest(openid);
