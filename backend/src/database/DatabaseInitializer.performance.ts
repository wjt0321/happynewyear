...
export class PerformanceDatabaseInitializer {
  ...
  constructor(database: SQLiteDatabase, batchSize: number = 100) {
    this.db = database;
   ...
   * Promise化的语句准备
   */
  private prepareStatement(sql: string): Promise<any> {
    return new Promise((resolve, reject) => {
   ...
   * Promise化的语句执行
   */
  private runStatement(stmt: any, params: any[]): Promise<void> {
    return new Promise((resolve, reject) => {
   ...
   * Promise化的语句释放
   */
  private finalizeStatement(stmt: any): Promise<void> {
    return new Promise((resolve, reject) => {
