...
interface DatabaseOperation {
  name: string;
  execute(db: sqlite3.Database): Promise<void>;
}
...
class TableCreationOperation implements DatabaseOperation {
  ...
  async execute(db: sqlite3.Database): Promise<void> {
...
class DataSeedingOperation implements DatabaseOperation {
   ...
   */
  private async batchInsertFortunes(db: sqlite3.Database): Promise<void> {
    return new Promise((resolve, reject) => {
      db.serialize(() => {
   ...
   * 获取现有运势数量
   */
  private getFortuneCount(db: sqlite3.Database): Promise<number> {
    return new Promise((resolve, reject) => {
